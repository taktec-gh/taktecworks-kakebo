...
| | |
|---|---|
| 状態 | **公開版 Step 2（サインアップ）が完了**（2026-09-19）。パスキーだけでアカウントを作れる。2アカウントでの画面での分離も確認済み |
| 次 | 未定（候補: デモアカウント、セキュリティヘッダー、リカバリーコード、デプロイ）。**スマホのパスキーでのクロスデバイス認証はデプロイ後のプレビュー環境で確認する**（Step 2 から持ち越し） |
| テスト | 1804件（`npx vitest run`）。すべて成功 |
