...
| | |
|---|---|
| 状態 | **公開版 Step 4（セキュリティヘッダー）が完了**（2026-09-19）。nonce 付きの厳格な CSP と HSTS などを全ページに付けた |
| 次 | 未定（候補: デプロイ、リカバリーコード）。**スマホのパスキーでのクロスデバイス認証はデプロイ後のプレビュー環境で確認する**（Step 2 から持ち越し） |
| テスト | 2914件（`npx vitest run`）。すべて成功 |
