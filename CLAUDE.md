...
| | |
|---|---|
| 状態 | **公開版 Step 3（デモアカウント）が完了**（2026-09-19）。「デモで試す」で24時間の使い捨てアカウントを作れる。期限切れは1日1回の Cron で削除 |
| 次 | 未定（候補: セキュリティヘッダー、デプロイ、リカバリーコード）。**スマホのパスキーでのクロスデバイス認証はデプロイ後のプレビュー環境で確認する**（Step 2 から持ち越し） |
| テスト | 2735件（`npx vitest run`）。すべて成功 |
...
アカウントは `/signup` で作る（ユーザー名もメールアドレスも持たない。アカウントはパスキーで決まる）。リカバリーコードはまだ無い。
